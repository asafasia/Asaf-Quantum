...
class qdac1():
    ...
    def defineFunctionGenerator(self, generator, waveform, period, dutycycle=50, repetitions=-1, trigger=0):
        ...
        if (waveform == Waveform.sine):
            str = self._checkForError(
                self._sendReceive(b"fun %d %d %f %d %d" % (generator, waveform, period, repetitions, trigger))).decode(
                "utf-8")
        ...
        if (waveform == Waveform.triangle or waveform == Waveform.square):
            str = self._checkForError(self._sendReceive(
                b"fun %d %d %d %f %d %d" % (generator, waveform, period, dutycycle, repetitions, trigger))).decode(
                "utf-8")
    ...
    def definePulsetrain(self, lowDuration, highDuration, lowVolts, highVolts, repetitions=-1, trigger=0):
        ...
        if not self.debugMode:
            ...
            if repetitions < -1 or repetitions > 0xFFFFFFFF:
                raise Exception("Invalid number of repetitions: %d" % repetitions)
        reply = str(self._checkForError(self._sendReceive(
            b"pul %d %d %e %e %d %d" % (lowDuration, highDuration, lowVolts, highVolts, repetitions, trigger))).decode(
            "utf-8"))
    ...
    def setChannelOutput(self, channel, generator, amplitude=1.0, offset=0):
        ...
        return self._checkForError(
            self._sendReceive(b"wav %d %d %e %e" % (channel, generator, amplitude, offset))).decode("utf-8")
    ...
    def setSyncOutput(self, syncChannel, generator, delay=0, pulseLength=1):
        ...
        res = self._checkForError(
            (self._sendReceive(b"syn %d %d %d %d" % (syncChannel, generator, delay, pulseLength))))
    ...
    def setVoltageCalibration(self, channel, theRange, samplesPerVolt, offsetSamples):
        ...
        return self._checkForError(
            self._sendReceive(b"vcal %d %d %e %e" % (channel, rangeFlag, samplesPerVolt, offsetSamples))).decode(
            "utf-8")
    ...
    def getVoltageCalibration(self, channel, theRange):
        ...
        return {"Gain": float(reply.split('mult')[1].split('offset')[0].strip()),
    ...
    def setCurrentCalibration(self, channel, theRange, microampsPerSample, offsetMicroamps):
        ...
        if theRange == 1e-6:
            ...
            raise Exception("Invalid current range %e" % theRange)
        return self._checkForError(
            self._sendReceive(b"ical %d %d %e %e" % (channel, rangeFlag, microampsPerSample, offsetMicroamps))).decode(
            "utf-8")
    ...
    def getCurrentCalibration(self, channel, theRange):
        ...
        if theRange == 1e-6:
            ...
            raise Exception("Invalid current range: %e" % theRange)
        reply = self._checkForError(self._sendReceive(b"ical %d %d" % (channel, rangeFlag))).decode(
            "utf-8")  # TODO: Proper formatting processing of response
        return {"Gain": float(reply.split('mult')[1].split('offset')[0].strip()),
                "Offset": float(reply.split('offset')[1].strip())}
...
def set_and_show_QDAC(qdac, channel, voltage):  # added by Naftali
    ...
    print("Measured voltage is %f" % qdac.getDCVoltage(channel))
    print("Measured current is %f" % qdac.getCurrentReading(channel))
...
#     # For channel 5:
#
#     with qdac(QDAC_port) as qdac:
#         qdac.setVoltageRange(5, 10)  # [V]
#         qdac.setCurrentRange(5, 1e-4)  # [A]
#         set_and_show_QDAC(qdac, qubit, flux_bias)


def play_flux_qdac(qubit, flux_bias):
    qubit_dict = {f'q{i}': i for i in range(1, 6)}
    print(qubit_dict)
    QDAC_port = 'COM4'
