...
if coupler == 'c13':
    ...
    raise ValueError('coupler not recognized: only ''c13'', ''c23'', ''c43'' and ''c53'' are valid couplers.')
    ...
    with exp_qspec.acquire_loop_rt(
        ...
        with exp_qspec.sweep(uid="qfreq_sweep", parameter=time_sweep):
            ...
            with exp_qspec.section(uid="readout_section", play_after="time_delay"):
                exp_qspec.play(signal=f"measure_{qubit_s}", pulse=readout_pulse(qubit_s),
                               phase=qubit_parameters[qubit_s]['angle'])

                exp_qspec.acquire(
                    signal="acquire_{qubit_s}",
                    handle="qubit_spec",
                    kernel=kernel
                )
