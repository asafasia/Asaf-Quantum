...
with exp_0.acquire_loop_rt(
    ...
    with exp_0.section():
        exp_0.play(signal="measure", pulse=readout_pulse(qubit))
        exp_0.acquire(
...
np.savetxt(f'traces_{qubit}.txt', traces, fmt='%s')
...
# for groundstate
plt.plot(time, np.imag(raw_0),alpha=0.5)
# plt.plot(time, np.imag(raw_0), "-b")
# # for excited state
plt.plot(time, np.imag(raw_1),alpha=0.5)
